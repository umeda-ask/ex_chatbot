...
// 連絡先などを保持（グローバル共有して常に存在させる）
if (!window.__chatForm) {
  window.__chatForm = { name:'', phone:'', email:'', preferred:'', path:[], freeTexts:[] };
}
const form = window.__chatForm;
...
function sendToGoogleSheet(payload) {
  return new Promise((resolve) => {
    try {
      ...
      form.method = 'POST';
      form.action = GAS_ENDPOINT; // 既存の /exec URL
      ...
      input.type = 'hidden';
      input.name = 'payload';
      input.value = JSON.stringify(payload); // ここに全部入れる
      ...
      // 送信できたら resolve（GASの戻り値は見ない：CORS不要のため）
      setTimeout(() => {
        // 後片付け
      ...
      console.error('[GAS] form post failed:', e);
      resolve(false);
    }
  });
}

...
document.addEventListener('DOMContentLoaded', () => {
  /* =========================
      0) 設定
  ========================= */
  // ★GASのWebアプリURL（デプロイ後の /exec ）に置き換え
  ...
  const chatList = {
    ...
    "q1": {
      ...
      "next":[ "q2","q7","q18","q23","q27","q29" ]
    },
    "q2": {
      "title": "ご相談内容はどんなことですか？",
      "choices": [ "遺言書について","相続について","遺産分割協議書について","贈与について","一つ前の質問に戻る" ],
      "next":[ "q3","q4","q5","q6" ]
    },
    "q3": {
    ...
    "q7": {
      ...
      "next":[ "q8","q9","q10","q11","q12","q13","q14","q15","q16","q17" ]
    },
    "q8": {
    ...
    "q18": {
      "title": "ご相談内容はどんなことですか？",
      "choices": [ "契約書について","内容証明について","離婚協議書について","その他の法務書類について","一つ前の質問に戻る" ],
      "next":[ "q19","q20","q21","q22" ]
    },
    "q19": {
    ...
    "q23": {
      "title": "ご相談内容はどんなことですか？",
      "choices": [ "創業・起業について","個人事業主について","法人設立について","一つ前の質問に戻る" ],
      "next":[ "q24","q25","q26" ]
    },
    "q24": {
    ...
    "q33": {
      ...
      "choices": [ "電話","メール","オンラインミーティング","対面ミーティング","一つ前の質問に戻る" ]
    },
  ...
  function scrollChatToBottom() {
    ...
    const items = messagesContainer.querySelectorAll('#chatbot-ul li');
    ...
    setTimeout(() => {
      ...
      if (key === 'q36') {
        ...
        current.choices.forEach((choice, i) => {
          ...
          btn.id = `q-${robotCount}-${i}`;
          btn.setAttribute('onclick', 'pushChoice(this)');
  ...
  window.pushChoice = function(e) {
    ...
    if (e.textContent === '一つ前の質問に戻る') {
      if (historyStack.length > 0) {
        ...
        if (userData.length > 0) { userData.pop(); userCount--; }
        robotOutput();
      }
      return;
    }
    ...
    for (let i = 0; i < current.choices.length; i++) {
      ...
      if (btn) { btn.disabled = true; btn.classList.add('choice-button-disabled'); }
    }
    ...
    // ★希望連絡方法（q33）
    if (currentKey === 'q33') {
      form.preferred = e.textContent; // 電話 / メール / オンラインミーティング / 対面ミーティング
    }
    ...
    if (current.next && current.next.length > choiceIndex) {
      ...
      const idx = chatKeys.indexOf(currentKey);
      nextKey = chatKeys[idx + 1];
    }
